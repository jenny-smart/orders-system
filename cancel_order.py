...
# Module: Cancel paid orders by phone + service date range
# Created: 2026-08-07
#
...
def _order_from_row(row) -> dict | None:
...
def find_paid_orders_for_cancel(
    ...
    """Search the backend with phone + service date range + paid status."""
    ...
    for page in range(1, max_pages + 1):
        ...
        for row in rows:
            ...
            if item["service_date"] and not (clean_date_s <= item["service_date"] <= clean_date_e):
                continue
            if item["payment_status"] and item["payment_status"] != "已付款":
                continue
...
def render_cancel_order(backend_email: str, backend_password: str, env_name: str):
    st.markdown("### 取消訂單")
    st.caption("依手機號碼＋服務日期區間搜尋『已付款』訂單；勾選後可批次取消。此功能獨立於 quick_order.py。")

    c1, c2, c3 = st.columns([1.2, 1, 1])
    with c1:
        phone = st.text_input("手機號碼", key="cancel_order_phone", placeholder="09xxxxxxxx")
    with c2:
        start_date = st.date_input("服務日期－起", value=date.today(), key="cancel_order_date_s")
    with c3:
        end_date = st.date_input("服務日期－迄", value=date.today(), key="cancel_order_date_e")

    st.text_input("付款狀態", value="已付款", disabled=True, key="cancel_order_paid_status")

    if st.button("🔎 搜尋已付款訂單", key="cancel_order_search", type="primary", use_container_width=True):
        if not backend_email or not backend_password:
            st.error("請先輸入後台帳號與密碼")
        else:
            try:
                with st.spinner("搜尋訂單中…"):
                    rows = find_paid_orders_for_cancel(
                        env_name,
                        backend_email,
                        backend_password,
                        phone,
                ...
                if not rows:
                    st.warning("查無符合『手機號碼＋服務日期區間＋已付款』的訂單")
            except Exception as exc:
    ...
    for i, row in enumerate(rows):
        label = (
            ...
            f"{row.get('name')} {row.get('phone')}　付款狀態：{row.get('payment_status') or '已付款'}"
        )
        checked = st.checkbox(
            label,
