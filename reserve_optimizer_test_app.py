...
.hero { background: linear-gradient(135deg, #FFFDF0 0%, #FFFBEA 100%); border: 1.5px solid var(--lemon-mid); border-radius: var(--radius); padding: 1.7rem 2.2rem 1.5rem; margin-bottom: 1.6rem; display: flex; align-items: center; gap: 1.1rem; box-shadow: 0 2px 12px rgba(245,197,24,0.10); }
.hero-emoji { font-size: 2.6rem; line-height: 1; }
.hero-title { font-family: 'Space Grotesk', sans-serif; font-size: 1.6rem; font-weight: 700; color: var(--charcoal); letter-spacing: -0.4px; }
.hero-sub { color: var(--ink); font-size: 0.9rem; margin-top: 0.25rem; opacity: 0.78; }
.hero-badge { margin-left: auto; font-size: 0.78rem; font-weight: 700; padding: 0.32rem 0.85rem; border-radius: 30px; white-space: nowrap; }
.hero-badge.on { background: rgba(52,199,89,0.12); color: #1F8A3B; border: 1.5px solid rgba(52,199,89,0.35); }
.hero-badge.off { background: rgba(142,142,147,0.12); color: var(--muted); border: 1.5px solid var(--border); }

.sec-label { font-size: 12px; font-weight: 700; color: var(--muted); letter-spacing: .04em; margin-bottom: 8px; }

[data-testid="stTextInput"] label, [data-testid="stNumberInput"] label, [data-testid="stSelectbox"] label, [data-testid="stMultiSelect"] label, [data-testid="stDateInput"] label, [data-testid="stRadio"] label, [data-testid="stSlider"] label, [data-testid="stCheckbox"] label { font-size: 13px !important; color: var(--ink) !important; font-weight: 700 !important; }
...
[data-testid="stButton"] > button { background: var(--lemon) !important; color: var(--charcoal) !important; border: none !important; border-radius: 10px !important; font-size: 15px !important; font-weight: 700 !important; padding: 0.55rem 1.2rem !important; box-shadow: 0 2px 10px rgba(245,197,24,0.28) !important; transition: transform 0.12s ease, background 0.12s ease; }
[data-testid="stButton"] > button:hover { background: var(--lemon-dark) !important; transform: translateY(-1px) !important; }
[data-testid="stButton"] > button:disabled { background: #D1D5DB !important; color: #777 !important; box-shadow: none !important; }
[data-testid="stButton"] > button[kind="primary"] { background: var(--charcoal) !important; color: var(--lemon) !important; box-shadow: 0 2px 10px rgba(28,28,30,0.22) !important; }
[data-testid="stButton"] > button[kind="primary"]:hover { background: #000 !important; }
...
[data-testid="stDataFrame"] { border: 1px solid var(--border) !important; border-radius: 12px !important; overflow: hidden !important; box-shadow: 0 2px 12px rgba(0,0,0,0.04); }
hr { border-color: #e8e8e8 !important; margin: 1.4rem 0 !important; }

/* 頁籤（分頁）— 質感款：底線＋膠囊高亮 */
[data-testid="stTabs"] [role="tablist"] { gap: 0.4rem; border-bottom: 1.5px solid var(--border); background: transparent; }
[data-testid="stTab"] { height: 46px; padding: 0 1.3rem; background: transparent; border-radius: 10px 10px 0 0; transition: color 0.15s ease, background 0.15s ease; }
[data-testid="stTab"] p { font-size: 15px !important; font-weight: 700 !important; color: var(--muted) !important; transition: color 0.15s ease; }
[data-testid="stTab"]:hover { background: rgba(245,197,24,0.08); }
[data-testid="stTab"]:hover p { color: var(--ink) !important; }
[data-testid="stTab"][aria-selected="true"] { background: var(--lemon-soft); }
[data-testid="stTab"][aria-selected="true"] p { color: var(--charcoal) !important; }
[data-testid="stTabs"] .react-aria-SelectionIndicator { background-color: var(--lemon) !important; height: 3px !important; border-radius: 3px 3px 0 0; }
[data-testid="stTabs"] [data-testid="stVerticalBlock"] { padding-top: 0.4rem; }

.step-card { background: white; border: 1px solid var(--border); border-radius: var(--radius); padding: 1.1rem 1.3rem; margin-top: 0.9rem; box-shadow: 0 2px 12px rgba(0,0,0,0.04); }
.step-card-title { font-size: 1rem; font-weight: 800; color: var(--charcoal); margin-bottom: 0.6rem; }
</style>
...
_lookup_ready = st.session_state.reserve_lookup is not None
st.markdown(f"""
<div class="hero">
  <div class="hero-emoji">🍋</div>
  <div>
    <div class="hero-title">大掃除保留單（測試機）</div>
    <div class="hero-sub">固定使用 dev 測試機。建立與取消都會真的異動測試機訂單，請先在「登入設定」完成登入。</div>
  </div>
  <div class="hero-badge {'on' if _lookup_ready else 'off'}">{'✅ 已登入' if _lookup_ready else '尚未登入'}</div>
</div>
""", unsafe_allow_html=True)

tab_login, tab_create, tab_cancel = st.tabs(["① 登入設定", "② 建立保留單", "③ 取消保留單"])

with tab_login:
    st.markdown('<div class="sec-label">後台帳號與保留會員</div>', unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    with c1:
    ...
    if st.button("登入並讀取保留會員", use_container_width=True, type="primary"):
        try:
    ...
    if st.session_state.reserve_lookup:
        st.success("已登入，可切換到「② 建立保留單」或「③ 取消保留單」頁籤繼續操作。")
    else:
        st.info("請輸入後台帳密與保留單會員手機後點擊登入。")

lookup = st.session_state.reserve_lookup

with tab_create:
    if not lookup:
        st.warning("請先於「① 登入設定」頁籤完成登入。")
    else:
        ...
        if not addresses:
            ...
            st.markdown('<div class="step-card"><div class="step-card-title">分析期間與保留率</div>', unsafe_allow_html=True)
            d1, d2 = st.columns(2)
            with d1:
