...
# Change Log
# v8.3
# - 配合 quick_order v8.3：若訂單有 2/3 位配班人員，排班修改時必須勾選 2/3 位不同檸檬人，不會用一般專員補位。
# v8.2
# - 配合 quick_order v8.2：檸檬人會依序補勾多位不同檸檬人，不再只看到檸檬人1。
# v8.1
# - 配合 quick_order v8.1：第二段補價差單沿用第一段原儲值金餘額，不受清零後餘額 0 影響。
# - 顯示檸檬人補勾結果時避免同一位檸檬人重複被視為多位。
# v8.0
# - 配合 quick_order v8.0：檸檬人清單解析新增 shift 頁掃描備援，避免顯示「找不到檸檬人清單」。
# - 2 人以上訂單會要求補勾相同人數的檸檬人。
# v7.9
# - 配合 quick_order v7.9：檸檬人勾班改走專員管理 /cleaner1 排班頁。
# - 同日已有衝突時段的檸檬人會跳過，改用下一位；例如已有全6不可再勾上4，已有下3可勾上4。
# v7.8
# - 修正儲值金清零說明與計算：優惠券A = 服務總額 - 儲值金餘額，不再用全額券。
# - 明確拆分介面：第一段用 /booking/stored_value_routine，第二段用 /booking/single。
# - 例：儲值金 1650、服務總額 4800 時，優惠券A 顯示 3150。
# v7.7
# - 儲值金補價差拆成兩段按鈕：先建立儲值金折抵單，再建立客付補價差單。
# - 日期類型改由服務日期自動判斷：週一到週五為平日，週六日為週末。
# - 儲值金折抵單使用「服務總額 - 儲值金餘額」作為優惠券A，剩餘額扣儲值金後歸零。
# v7.6
...
# =========================================================
    ...
    # -----------------------------------------------------
        ...
        st.markdown(
            f'<div class="hint-box">儲值金清零訂單會送到 <b>/booking/stored_value_routine</b>。服務總額為 <b>{sv_unit_price} × {sv_person_hours} = {sv_stored_total_preview}</b>；優惠券A會用「服務總額 - 儲值金餘額」計算，剩餘金額由儲值金扣抵後歸零。</div>',
            unsafe_allow_html=True,
        )
