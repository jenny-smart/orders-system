...
FUNCTION_OPTIONS = [
    ...
    ("服務異動：車馬費／異動費、服務前後加減時、退款／客訴退款／物損退款，"
     "分階段查詢試算後回填後台。",
     "memo", "服務異動"),
    ("財政部電子發票取號：畫面引導客服至財政部電子發票平台查詢／下載字軌號碼，上傳後自動命名並存入"
     "Google Drive 對應資料夾，同步寫入執行紀錄。",
     "orders", "財政部電子發票取號"),
]
...
# =========================================================
if mode == "財政部電子發票取號":
    from einvoice_serial import render_einvoice_serial

    step("3", "財政部電子發票取號")
    info_panel("功能說明", [
        "登入財政部電子發票平台（含圖形驗證碼）由客服自行在瀏覽器操作，本系統不經手、不儲存密碼。",
        "選擇區域後，畫面會提示該用哪組統一編號登入，以及目前該查詢哪個發票期別。",
        "客服在財政部平台查詢、下載 CSV 後，回來這裡上傳，系統會自動依規則重新命名。",
        "檔案會自動存入 Google Drive 對應區域的「紙本發票／期別」資料夾，找不到期別資料夾會自動新增。",
        "存檔完成後會同步在「Jenny's Lemonhometools」試算表的「執行紀錄」分頁補一筆紀錄。",
    ])
    render_einvoice_serial()
    st.stop()
